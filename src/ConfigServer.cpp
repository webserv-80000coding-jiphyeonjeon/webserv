...
ConfigServer::ConfigServer()
    : server_name_(), listen_(), location_(), common_config_() {}
...
const ConfigServer::ListenListType& ConfigServer::getListen() const {
  return listen_;
}

const ConfigServer::LocationType& ConfigServer::getLocation() const {
  return location_;
}

const ConfigServer::Common& ConfigServer::getCommon() const {
...
const ConfigServer::ClientBodyBufferSizeType&
ConfigServer::getClientBodyBufferSize() const {
...
const ConfigServer::IndexType& ConfigServer::getIndex() const {
...
void ConfigServer::addServerName(const std::string& value) {
  server_name_.push_back(value);
}

void ConfigServer::setListen(const ConfigServer::ListenListType& listen) {
  listen_ = listen;
}

void ConfigServer::addListen(const ConfigServer::ListenType& value) {
  for (ListenListType::iterator it = listen_.begin(); it != listen_.end();
       ++it) {
    if (*it == value)
      throw std::invalid_argument(
          "Config: listen: Duplicate host:port is not allowed.");
  }

  listen_.push_back(value);
}

void ConfigServer::setLocation(const ConfigServer::LocationType& location) {
  location_ = location;
}
...
void ConfigServer::setCommon(const ConfigServer::Common& common) {
...
void ConfigServer::setClientBodyBufferSize(
    const ConfigServer::ClientBodyBufferSizeType& value) {
